...
`make e2e` builds the frontend, migrates a scratch SQLite file, starts the API on port 8000, and runs the one Playwright smoke test (log in, home renders). CI runs it as a job alongside `make check`. It fails if port 8000 is already in use by another process, such as a local dev server started per the section below.
