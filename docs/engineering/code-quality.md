...
- **Domain rules import nothing from FastAPI, SQLAlchemy, or IO.** FIFO, cost split, settlement, and profit are plain functions over plain data. Enforce with a pytest test that runs the check in a fresh subprocess and imports **every** domain submodule via `pkgutil.walk_packages`, not just the package `__init__` — importing the package alone only executes `<domain>/__init__.py`, so a submodule's own `import sqlalchemy` stays invisible and the check passes vacuously forever. For example, a subprocess script that walks and imports every submodule under `<domain>`, then raises `SystemExit` (not `assert`, which `python -O` strips) if `'fastapi'` or `'sqlalchemy'` ends up in `sys.modules`. Run it in a fresh subprocess rather than asserting on the current process's `sys.modules`, so another test importing FastAPI or SQLAlchemy first can't taint this check with a false failure regardless of suite order.
- **No field anywhere is a float.** Money is `int` cents (named `*_cents`); weights and quantities are also integers (see `data-model.md`). Never `float` or `Decimal`. Enforced by a test that enumerates every field the data model lists as money, weight, or quantity — not by name suffix, since most quantity fields don't carry one (see `acceptance.md`'s enumerated list) — across SQLAlchemy columns (via `Base.registry.mappers`), Pydantic fields, and domain dataclass fields (a subclass walk filtered to project modules, since a plain dataclass appears in neither the mapper registry nor a Pydantic model list), and asserts the exact type is `int`; asserts no column, field, or dataclass attribute anywhere is `float` or `Decimal`; plus pyright strict and the OpenAPI integer assertion in `acceptance.md`.
