...
- **No field anywhere is a float.** Money is `int` cents (named `*_cents`); weights and quantities are also integers (see `data-model.md`). Never `float` or `Decimal`. Enforced by a test that enumerates every SQLAlchemy column and Pydantic field named `*_cents`, `*_weight`, `*_count`, or `quantity` and asserts the exact type is `int`, and asserts no column or field anywhere is `float` or `Decimal`; plus pyright strict and the OpenAPI integer assertion in `acceptance.md`.
