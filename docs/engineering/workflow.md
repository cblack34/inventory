...
Repository facts: `cblack34/inventory`, public. **Copilot code review is not currently available here.** Verified 2026-09-13 on PRs #5 and #6: `gh api -X POST repos/{owner}/{repo}/pulls/<pr-number>/requested_reviewers -f 'reviewers[]=Copilot'` returns an empty reviewer list and records no `review_requested` event, and `gh pr edit --add-reviewer Copilot` fails with "Could not resolve user with login 'copilot'". Until the owner enables Copilot code review for the repository and updates this paragraph, the `review-pr` fallback is the first reviewer. When Copilot is enabled, it reviews automatically on PR open, and the `gh api` command above requests another pass; an empty reviewer list in that response is normal and the review arrives asynchronously within several minutes. In this environment the fallback skills are named `pr-review:pr-review` (review) and `pr-review:address-pr-review` (address comments); use them where this document says `review-pr` and `address-pr-review`.
