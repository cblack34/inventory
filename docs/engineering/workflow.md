...
Repository facts: `cblack34/inventory`, public, GitHub Copilot code review enabled per the owner. Copilot reviewed PR #1 automatically on open. Working re-request method: `gh api -X POST repos/{owner}/{repo}/pulls/{n}/requested_reviewers -f 'reviewers[]=Copilot'` (the bot's plain login, not the `[bot]`-suffixed form). The call returns an empty reviewer list — that is the normal response, not a failure — and Copilot posts its review several minutes later (observed: about seven minutes), not synchronously; wait before concluding the request failed. `gh pr edit --add-reviewer` still fails to resolve the bot login, so use the REST call above instead. In this environment the fallback skills are named `pr-review:pr-review` (review) and `pr-review:address-pr-review` (address comments); use them where this document says `review-pr` and `address-pr-review`.
...
CI is GitHub Actions, one workflow at `.github/workflows/ci.yml`, running `make check` and `make e2e` on every pull request. No other jobs until something consumes their output.
