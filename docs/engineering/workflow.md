...
Repository facts: `cblack34/inventory`, public, GitHub Copilot code review enabled per the owner. Copilot reviewed PR #1 automatically on open. Working re-request method: `gh api -X POST repos/{owner}/{repo}/pulls/<pr-number>/requested_reviewers -f 'reviewers[]=Copilot'` — substitute the literal PR number; `gh api` expands only the `{owner}`, `{repo}`, and `{branch}` placeholders, and a literal `{n}` returns 404. Use the bot's plain login `Copilot`, not the `[bot]`-suffixed form. The call returns an empty reviewer list — that is the normal response, not a failure — and Copilot posts its review several minutes later (observed: about seven minutes), not synchronously; wait before concluding the request failed. `gh pr edit --add-reviewer @copilot` failed to resolve the bot login on 2026-09-12 using the `[bot]`-suffixed form; a reviewer reports it working with the plain login `Copilot`, unverified here — don't re-test it, since a successful call requests a live review. Prefer the REST call above until that is independently confirmed. In this environment the fallback skills are named `pr-review:pr-review` (review) and `pr-review:address-pr-review` (address comments); use them where this document says `review-pr` and `address-pr-review`.
