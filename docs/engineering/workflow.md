...
Repository facts: `cblack34/inventory`, public, GitHub Copilot code review enabled. The `protect-default` ruleset auto-requests Copilot only on PRs whose base is `main`, so a spine PR gets a review on open but a leaf PR into `slice/<name>` does not. Request it on every leaf PR, and for another pass on any PR, with `gh api -X POST repos/{owner}/{repo}/pulls/<pr-number>/requested_reviewers -f 'reviewers[]=copilot-pull-request-reviewer[bot]'` using the literal PR number (only `{owner}` and `{repo}` are `gh api` placeholders). The login `Copilot` and `gh pr edit --add-reviewer` do not resolve (verified 2026-09-13 on PRs #5 and #6); the bot identifier above records a `review_requested` event immediately. The response's reviewer list is empty, which is normal; the review arrives asynchronously, typically within several minutes. If the owner extends the ruleset to `slice/*`, leaf PRs will also be reviewed on open, and this paragraph should say so. In this environment the fallback skills are named `pr-review:pr-review` (review) and `pr-review:address-pr-review` (address comments); use them where this document says `review-pr` and `address-pr-review`.
...
- a migration would drop, rewrite, or backfill movement or batch-cost rows once real data exists; propose the compensating-movement alternative instead.
