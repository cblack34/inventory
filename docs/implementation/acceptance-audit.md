...
and their rows below now read `none`. `make check` exits 0 on that head
...
5. **Human items (as `docs/acceptance.md` itself designates them), not gaps in the code:** the owner has not yet (per this audit's evidence) run the documented backup command once against a live instance and confirmed a copy exists, and has not yet opened every screen at 375px on a phone.
