...
- [ ] Stock removals are serialized: two concurrent removals whose sum exceeds on-hand cannot both succeed. _Automated check:_ two threads each remove more than half of a batch's on-hand at once; exactly one succeeds and stock is never negative.
