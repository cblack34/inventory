...
- **Every movement belongs to exactly one entry (a visit or a manual operation).** A visit is an entry, so every movement it creates references that visit and profit per visit is one query. A manual operation is also an entry and may contain several movement rows, one per batch FIFO selected.
