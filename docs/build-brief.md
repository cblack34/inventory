...
- **Multi-size yield.** A recipe yields several sizes (for example large bag, small bag, sample). Each size has a portion weight and a sale price. A sample is an ordinary size with a zero price; nothing special-cases it. The recipe stores a typical yield per size, used both as a prefill at bake time and as the weight denominator of the recipe cost estimate shown before any bake exists.
...
- **Market visit.** Enter units taken from the kitchen, revenue as one total, booth fee, units returned to the kitchen, and units tossed. Sample (zero-price) returns prefill to zero and are editable; priced-size returns are required with no prefill. Missing units are Sold or Sampled by the same price rule.
...
- **Locations are data.** Kitchen, Sold, Waste, Sampled, and Production are built-in rows created by migration and cannot be deleted, renamed, or deactivated. Stands and markets are user rows with a type. Adding a stand is a row, not a deploy.
...
- **The API is versioned under `/api/v1` and is resource-shaped, not action-shaped.** Every entry kind is a noun: a bake is `POST /api/v1/batches`, a stand or market visit is `POST /api/v1/visits` with the kind in the payload, a manual move or toss is `POST /api/v1/movements`, and undo is `POST /api/v1/reversals` naming the entry it reverses. History is `GET /api/v1/entries`, derived stock is `GET /api/v1/stock`, login is `POST /api/v1/session`, the business date is `GET /api/v1/today`, and ingredients, recipes, and locations are ordinary collections. No route is a verb. Reason: the data model already names these as entry kinds, and a reversal resource satisfies "undo is its own endpoint, not an entry update" without an RPC-style route. Every error response is an RFC 9457 Problem Details document (`application/problem+json`) with `type`, `title`, `status`, `detail`, and `instance`; a domain rejection carries the error's fields (for example `location_id`, `size_id`, `on_hand`, `requested`) as extension members, and a validation rejection carries an `errors` list. The problem schema is declared on error responses in the OpenAPI document so the generated frontend types include it.
...
- **SQLite on a single volume.** Disk loss is data loss. Mitigation: documented nightly `sqlite3 .backup` to object storage, and the backup command is part of the deployment docs. Accepted risk owner: the user.
