...
| Nightly host cron runs `docker compose exec app sqlite3 "$DB" ".backup '/backups/inventory.db'"` with `/backups` bind-mounted from the host, then uploads that host file to object storage | adopted | The `sqlite3` CLI and the database path live in the container; cron and object-storage credentials live on the host. The bind mount is the boundary. |
