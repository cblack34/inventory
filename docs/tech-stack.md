...
| `uvicorn` | adopted | The ASGI server FastAPI's documentation runs under. Behind Caddy it must trust the proxy's forwarded headers, otherwise every request appears to come from the proxy and the per-IP login throttle collapses into one shared counter. `uvicorn.run()` enables proxy headers by default and reads the trusted address from the `FORWARDED_ALLOW_IPS` environment variable, so the deployment notes set that variable rather than CLI flags (verified against the pinned version during the deploy-local slice). |
...
| GitHub Actions running `make check` and `make e2e` as two parallel jobs on every PR | adopted | Repository is `cblack34/inventory` on GitHub. |
